...
<?php
	...
	$query = $db->query("select * from actor order by a_name $name limit $start, $listSize");

	while ($row = $query->fetch()) {
?> 
<div style="text-align:center; margin-top:100px">
	<div style="float: left; width: 25%; margin-bottom: 50px">
		<a href="actor.php?a_name=<?=$row["a_name"]?>&page=<?=$page?>"><img src = '<?=$row["a_pic"]?>'style="height:450px;"/></a><br><?=$row["a_name"]?>
	</div>
</div>
<?php
	}	// while
...
<a href="a_write.php?page=<?=$page?>">배우 추가</a>
</div>
...
<?php
	...
	$query = $db->query("select count(*) from actor");
	$row = $query->fetch();
	$numRecords = $row[0];
	...
	if($firstLink > 1){
		$link = $firstLink - 1;
		echo "<a href=\"actorlist.php?page=$link\">이전 </a>";
	}
	
	for($i = $firstLink; $i <= $lastLink; $i++){
		if($i == $page){
			echo "<a href=\"actorlist.php?page=$i\"><u>$i</u></a>";
		}else{
			echo "<a href=\"actorlist.php?page=$i\"> $i </a>";
		}
	}
	
	if($lastLink < $numPages){
		$link = $lastLink + 1;
		echo "<a href=\"actorlist.php?page=$link\"> 다음</a>";
	}
